 ...
 * {
 *   noQualifyingFlights: boolean,   // true -> whole block written as '无'
 *   muShown: boolean,
 *   muPrice: number,
...
async function writeResults(inputXlsxPath, outputXlsxPath, resultsByRow) {
  ...
  for (const [rowNumber, result] of Object.entries(resultsByRow)) {
    ...
    if (result.noQualifyingFlights) {
      // No flight on this route had <= 1 stop -- mark the whole block
      // "无" rather than "东航未展示", which would otherwise be
      // indistinguishable from "MU just wasn't in the results".
      for (const col of Object.values(TRIP_COLS)) row.getCell(col).value = '无';
      row.commit();
      continue;
    }
